...
struct StableBTreeMap0ValueType(Employee);
impl CdkActTryIntoVmValue<&mut boa_engine::Context<'_>, boa_engine::JsValue>
...
#[ic_cdk_macros::query(name = "searchEmployees")]
#[candid::candid_method(query, rename = "searchEmployees")]
async fn _cdk_user_defined_searchEmployees(
    _cdk_user_defined_query: String,
) -> (_AzleResult<Vec<Employee>, String>) {
    unwrap_or_trap(|| {
        BOA_CONTEXT_REF_CELL
            .with(|boa_context_ref_cell| {
                let mut boa_context = boa_context_ref_cell.borrow_mut();
                ...
                METHOD_NAME_REF_CELL
                    .with(|method_name_ref_cell| {
                        let mut method_name_mut = method_name_ref_cell.borrow_mut();
                        *method_name_mut = "searchEmployees".to_string();
                    });
                MANUAL_REF_CELL
                    .with(|manual_ref_cell| {
                        let mut manual_mut = manual_ref_cell.borrow_mut();
...
#[ic_cdk_macros::query(name = "getEmployees")]
#[candid::candid_method(query, rename = "getEmployees")]
async fn _cdk_user_defined_getEmployees() -> (_AzleResult<Vec<Employee>, String>) {
    unwrap_or_trap(|| {
        BOA_CONTEXT_REF_CELL
            .with(|boa_context_ref_cell| {
                let mut boa_context = boa_context_ref_cell.borrow_mut();
                ...
                METHOD_NAME_REF_CELL
                    .with(|method_name_ref_cell| {
                        let mut method_name_mut = method_name_ref_cell.borrow_mut();
                        *method_name_mut = "getEmployees".to_string();
                    });
                MANUAL_REF_CELL
                    .with(|manual_ref_cell| {
                        let mut manual_mut = manual_ref_cell.borrow_mut();
                ...
                let function_js_value = exports_js_object
                    .get("getEmployees", &mut boa_context)?;
                let function_js_object = function_js_value
                    .as_object()
                    .ok_or_else(|| RuntimeError::ReferenceError(
                        format!("{} is not defined", "getEmployees"),
                    ))?;
                let boa_return_value = function_js_object
                    .call(&boa_engine::JsValue::Null, &[], &mut boa_context)?;
                let final_return_value = async_await_result_handler(
...
#[ic_cdk_macros::query(name = "getEmployee")]
#[candid::candid_method(query, rename = "getEmployee")]
async fn _cdk_user_defined_getEmployee(
    _cdk_user_defined_id: String,
) -> (_AzleResult<Employee, String>) {
    unwrap_or_trap(|| {
        BOA_CONTEXT_REF_CELL
            .with(|boa_context_ref_cell| {
                let mut boa_context = boa_context_ref_cell.borrow_mut();
                ...
                METHOD_NAME_REF_CELL
                    .with(|method_name_ref_cell| {
                        let mut method_name_mut = method_name_ref_cell.borrow_mut();
                        *method_name_mut = "getEmployee".to_string();
                    });
                MANUAL_REF_CELL
                    .with(|manual_ref_cell| {
                        let mut manual_mut = manual_ref_cell.borrow_mut();
                ...
                let function_js_value = exports_js_object
                    .get("getEmployee", &mut boa_context)?;
                let function_js_object = function_js_value
                    .as_object()
                    .ok_or_else(|| RuntimeError::ReferenceError(
                        format!("{} is not defined", "getEmployee"),
                    ))?;
                let boa_return_value = function_js_object
                    .call(
                        &boa_engine::JsValue::Null,
...
#[ic_cdk_macros::update(name = "hireEmployee")]
#[candid::candid_method(update, rename = "hireEmployee")]
async fn _cdk_user_defined_hireEmployee(
    _cdk_user_defined_employee: Employee,
) -> (_AzleResult<Employee, String>) {
    unwrap_or_trap(|| {
        BOA_CONTEXT_REF_CELL
            .with(|boa_context_ref_cell| {
                ...
                METHOD_NAME_REF_CELL
                    .with(|method_name_ref_cell| {
                        let mut method_name_mut = method_name_ref_cell.borrow_mut();
                        *method_name_mut = "hireEmployee".to_string();
                    });
                MANUAL_REF_CELL
                    .with(|manual_ref_cell| {
                        let mut manual_mut = manual_ref_cell.borrow_mut();
                ...
                let function_js_value = exports_js_object
                    .get("hireEmployee", &mut boa_context)?;
                let function_js_object = function_js_value
                    .as_object()
                    .ok_or_else(|| RuntimeError::ReferenceError(
                        format!("{} is not defined", "hireEmployee"),
                    ))?;
                let boa_return_value = function_js_object
                    .call(
                        &boa_engine::JsValue::Null,
                        &[
                            _cdk_user_defined_employee
                                .try_into_vm_value(&mut boa_context)?,
                        ],
                        &mut boa_context,
                    )?;
...
#[ic_cdk_macros::update(name = "fireEmployee")]
#[candid::candid_method(update, rename = "fireEmployee")]
async fn _cdk_user_defined_fireEmployee(
    _cdk_user_defined_id: String,
) -> (_AzleResult<Employee, String>) {
    unwrap_or_trap(|| {
        BOA_CONTEXT_REF_CELL
            .with(|boa_context_ref_cell| {
                ...
                METHOD_NAME_REF_CELL
                    .with(|method_name_ref_cell| {
                        let mut method_name_mut = method_name_ref_cell.borrow_mut();
                        *method_name_mut = "fireEmployee".to_string();
                    });
                MANUAL_REF_CELL
                    .with(|manual_ref_cell| {
                        let mut manual_mut = manual_ref_cell.borrow_mut();
                ...
                let function_js_value = exports_js_object
                    .get("fireEmployee", &mut boa_context)?;
                let function_js_object = function_js_value
                    .as_object()
                    .ok_or_else(|| RuntimeError::ReferenceError(
                        format!("{} is not defined", "fireEmployee"),
                    ))?;
                let boa_return_value = function_js_object
                    .call(
                        &boa_engine::JsValue::Null,
...
#[ic_cdk_macros::update(name = "updateEmployee")]
#[candid::candid_method(update, rename = "updateEmployee")]
async fn _cdk_user_defined_updateEmployee(
    _cdk_user_defined_id: String,
    _cdk_user_defined_employee: Employee,
) -> (_AzleResult<Employee, String>) {
    unwrap_or_trap(|| {
        BOA_CONTEXT_REF_CELL
            .with(|boa_context_ref_cell| {
                ...
                METHOD_NAME_REF_CELL
                    .with(|method_name_ref_cell| {
                        let mut method_name_mut = method_name_ref_cell.borrow_mut();
                        *method_name_mut = "updateEmployee".to_string();
                    });
                MANUAL_REF_CELL
                    .with(|manual_ref_cell| {
                        let mut manual_mut = manual_ref_cell.borrow_mut();
                ...
                let function_js_value = exports_js_object
                    .get("updateEmployee", &mut boa_context)?;
                let function_js_object = function_js_value
                    .as_object()
                    .ok_or_else(|| RuntimeError::ReferenceError(
                        format!("{} is not defined", "updateEmployee"),
                    ))?;
                let boa_return_value = function_js_object
                    .call(
                        &boa_engine::JsValue::Null,
...
#[ic_cdk_macros::update(name = "deleteEmployee")]
#[candid::candid_method(update, rename = "deleteEmployee")]
async fn _cdk_user_defined_deleteEmployee(
    _cdk_user_defined_id: String,
) -> (_AzleResult<Option<Employee>, String>) {
    unwrap_or_trap(|| {
        BOA_CONTEXT_REF_CELL
            .with(|boa_context_ref_cell| {
                let mut boa_context = boa_context_ref_cell.borrow_mut();
                ...
                METHOD_NAME_REF_CELL
                    .with(|method_name_ref_cell| {
                        let mut method_name_mut = method_name_ref_cell.borrow_mut();
                        *method_name_mut = "deleteEmployee".to_string();
                    });
                MANUAL_REF_CELL
                    .with(|manual_ref_cell| {
                        let mut manual_mut = manual_ref_cell.borrow_mut();
                ...
                let function_js_value = exports_js_object
                    .get("deleteEmployee", &mut boa_context)?;
                let function_js_object = function_js_value
                    .as_object()
                    .ok_or_else(|| RuntimeError::ReferenceError(
                        format!("{} is not defined", "deleteEmployee"),
                    ))?;
                let boa_return_value = function_js_object
                    .call(
                        &boa_engine::JsValue::Null,
...
#[ic_cdk_macros::update(name = "promoteEmployee")]
#[candid::candid_method(update, rename = "promoteEmployee")]
async fn _cdk_user_defined_promoteEmployee(
    _cdk_user_defined_id: String,
) -> (_AzleResult<Employee, String>) {
    unwrap_or_trap(|| {
        BOA_CONTEXT_REF_CELL
            .with(|boa_context_ref_cell| {
                ...
                METHOD_NAME_REF_CELL
                    .with(|method_name_ref_cell| {
                        let mut method_name_mut = method_name_ref_cell.borrow_mut();
                        *method_name_mut = "promoteEmployee".to_string();
                    });
                MANUAL_REF_CELL
                    .with(|manual_ref_cell| {
                        let mut manual_mut = manual_ref_cell.borrow_mut();
                ...
                let function_js_value = exports_js_object
                    .get("promoteEmployee", &mut boa_context)?;
                let function_js_object = function_js_value
                    .as_object()
                    .ok_or_else(|| RuntimeError::ReferenceError(
                        format!("{} is not defined", "promoteEmployee"),
                    ))?;
                let boa_return_value = function_js_object
                    .call(
                        &boa_engine::JsValue::Null,
...
#[ic_cdk_macros::update(name = "demoteEmployee")]
#[candid::candid_method(update, rename = "demoteEmployee")]
async fn _cdk_user_defined_demoteEmployee(
    _cdk_user_defined_id: String,
) -> (_AzleResult<Employee, String>) {
    unwrap_or_trap(|| {
        BOA_CONTEXT_REF_CELL
            .with(|boa_context_ref_cell| {
                ...
                METHOD_NAME_REF_CELL
                    .with(|method_name_ref_cell| {
                        let mut method_name_mut = method_name_ref_cell.borrow_mut();
                        *method_name_mut = "demoteEmployee".to_string();
                    });
                MANUAL_REF_CELL
                    .with(|manual_ref_cell| {
                        let mut manual_mut = manual_ref_cell.borrow_mut();
                ...
                let function_js_value = exports_js_object
                    .get("demoteEmployee", &mut boa_context)?;
                let function_js_object = function_js_value
                    .as_object()
                    .ok_or_else(|| RuntimeError::ReferenceError(
                        format!("{} is not defined", "demoteEmployee"),
                    ))?;
                let boa_return_value = function_js_object
                    .call(
                        &boa_engine::JsValue::Null,
...
#[ic_cdk_macros::update(name = "increaseSalary")]
#[candid::candid_method(update, rename = "increaseSalary")]
async fn _cdk_user_defined_increaseSalary(
    _cdk_user_defined_id: String,
    _cdk_user_defined_amount: _CdkFloat64,
) -> (_AzleResult<Employee, String>) {
    unwrap_or_trap(|| {
        BOA_CONTEXT_REF_CELL
            .with(|boa_context_ref_cell| {
                ...
                METHOD_NAME_REF_CELL
                    .with(|method_name_ref_cell| {
                        let mut method_name_mut = method_name_ref_cell.borrow_mut();
                        *method_name_mut = "increaseSalary".to_string();
                    });
                MANUAL_REF_CELL
                    .with(|manual_ref_cell| {
                        let mut manual_mut = manual_ref_cell.borrow_mut();
                ...
                let function_js_value = exports_js_object
                    .get("increaseSalary", &mut boa_context)?;
                let function_js_object = function_js_value
                    .as_object()
                    .ok_or_else(|| RuntimeError::ReferenceError(
                        format!("{} is not defined", "increaseSalary"),
                    ))?;
                let boa_return_value = function_js_object
                    .call(
                        &boa_engine::JsValue::Null,
                        &[
                            _cdk_user_defined_id.try_into_vm_value(&mut boa_context)?,
                            _cdk_user_defined_amount.try_into_vm_value(&mut boa_context)?,
                        ],
                        &mut boa_context,
                    )?;
                let final_return_value = async_await_result_handler(
...
struct Employee {
    id: Box<String>,
    firstName: Box<String>,
    lastName: Box<String>,
    position: Box<String>,
    department: Box<String>,
    salary: Box<_CdkFloat64>,
    isEmployed: Box<bool>,
